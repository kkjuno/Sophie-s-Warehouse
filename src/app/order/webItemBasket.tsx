...
export default function WebItemBasket() {
    ...
    <>
      ...
      <div className={order_styles.web_order_item_basket_section}>
        ...
        <div className={order_styles.web_order_item_list}>
          {items.length > 0 ? (
            items.map((item) => (
              <div key={item.id} className={order_styles.web_order_item_area}>
                {/* 상품 이미지 영역 */}
                ...
                <div className={order_styles.web_order_item_info}>
                  <div className={order_styles.web_order_item_name}>
                    ...
                    {(item.color || item.size) && (
                      <div className={order_styles.web_order_item_options}>
                  ...
                  <div className={order_styles.web_order_basket_item_count_wrapper}>
                    <span className={order_styles.web_order_basket_item_count}>
                      {item.quantity}개
                    </span>
                  </div>
                  {/* 가격 */}
                  <div className={order_styles.web_order_basket_item_price_wrapper}>
                    <span>{formatPrice((item.price || 0) * (item.quantity || 1))}원</span>
                  </div>
                </div>
              </div>
            ))
      ...
      {/* 장바구니 가기 버튼 */}
      <Link className={order_styles.web_order_go_shopping_cart_button_wrapper} href="/shoppingcart">
