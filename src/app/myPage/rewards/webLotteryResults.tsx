...
export default function WebLotteryResults() {
  ...
  return (
    <div className={rewardsStyles.web_lottery_result_section}>
      <h2>당첨 내역</h2>
      <div className={rewardsStyles.web_lottery_result_header}>
        <div>날짜/당첨번호</div>
        <div>상품명/옵션</div>
        <div>상품금액</div>
        <div>진행상태</div>
      ...
      {lotteryPosts.length > 0 ? (
        lotteryPosts.map((post) => (
          <div key={post._id} className={rewardsStyles.web_lottery_result_row}>
            <div>{post.createdAt.slice(0, 10)}</div>
            <div className={rewardsStyles.web_item_name}>{post.title}</div>
        ...
        <div className={rewardsStyles.web_lottery_result_row_text}>당첨 내역이 없습니다.</div>
      )}
    </div>
  );
}
