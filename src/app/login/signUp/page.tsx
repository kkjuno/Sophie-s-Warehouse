import '@/styles/globals.css';
import signUp_styles from '@/styles/login/signUp.module.css';
import styles from '@/styles/components/button.module.css';
import Image from 'next/image';

export default function signUpPage() {
  return (
    <>
      {/* 모바일 회원가입 페이지 */}
      <div className={signUp_styles.mobile_signUp_page}>
        {/* 모바일 회원가입 페이지 헤더 */}
        <Image
          className={signUp_styles.mobile_logo}
          src="/logo/소피의창고 로고.svg"
          alt="소피의 창고 로고"
          width={173}
          height={82}
        />
        <span className={signUp_styles.mobile_description}>
          &quot;아직 문은 잠겨 있어요. 열쇠는 &quot;
          <br></br>
          <strong className={signUp_styles.mobile_description_point}>&quot;회원가입&quot;</strong>에
          있답니다.
        </span>

        {/* 모바일 회원가입 페이지 박스 */}
        <div className={signUp_styles.mobile_form_wrapper}>
          {/* 모바일 회원가입 페이지 뒤로가기와 회원가입 제목 */}
          <div className={signUp_styles.mobile_signUp_title_container}>
            <svg
              className={signUp_styles.mobile_back_button}
              width="15"
              height="15"
              viewBox="0 0 15 15"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              xlinkHref="http://www.w3.org/1999/xlink"
            >
              <rect
                x="15"
                y="15"
                width="15"
                height="15"
                transform="rotate(180 15 15)"
                fill="url(#pattern0_311_1505)"
              />
              <defs>
                <pattern
                  id="pattern0_311_1505"
                  patternContentUnits="objectBoundingBox"
                  width="1"
                  height="1"
                >
                  <use xlinkHref="#image0_311_1505" transform="scale(0.00195312)" />
                </pattern>
                <image
                  id="image0_311_1505"
                  width="512"
                  height="512"
                  preserveAspectRatio="none"
                  xlinkHref="data:image/png;base64,"
                />
              </defs>
            </svg>
            <span className={signUp_styles.mobile_signUp_title}>회원가입</span>
          </div>
          {/* 모바일 회원가입 페이지 인풋 */}

          <form className={signUp_styles.mobile_form}>
            <div className={signUp_styles.mobile_label_top}>
              <div className={signUp_styles.mobile_input_name_wrapper}>
                <label className={signUp_styles.mobile_label} htmlFor="email">
                  이름 *
                </label>
                <input className={signUp_styles.mobile_input_name} type="text" id="name" />
              </div>
              <div className={signUp_styles.mobile_input_name_wrapper}>
                <label className={signUp_styles.mobile_label} htmlFor="email">
                  휴대폰 번호 *
                </label>
                <input className={signUp_styles.mobile_input_pn} type="text" id="phonenumber" />
              </div>
            </div>

            <label className={signUp_styles.mobile_label} htmlFor="email">
              이메일 주소 *
            </label>
            <input
              className={signUp_styles.mobile_input}
              type="email"
              id="email"
              placeholder="example@example.com"
            />

            <label className={signUp_styles.mobile_label} htmlFor="password">
              비밀번호 *
            </label>
            <input className={signUp_styles.mobile_input} type="password" id="password" />

            <label className={signUp_styles.mobile_label} htmlFor="password">
              비밀번호 확인 *
            </label>
            <input className={signUp_styles.mobile_input} type="password" id="password" />

            <div className={signUp_styles.mobile_checkbox_container}>
              <div className={signUp_styles.mobile_checkbox_wrapper}>
                <input
                  type="checkbox"
                  id="option"
                  name="option"
                  className={signUp_styles.mobile_checkbox}
                />
                <label htmlFor="option" className={signUp_styles.mobile_checkbox_label}>
                  <strong className={signUp_styles.mobile_description_point}>이용약관</strong>에
                  동의합니다.
                </label>
              </div>

              <div className={signUp_styles.mobile_checkbox_wrapper}>
                <input
                  type="checkbox"
                  id="option"
                  name="option"
                  className={signUp_styles.mobile_checkbox}
                />
                <label htmlFor="option" className={signUp_styles.mobile_checkbox_label}>
                  <strong className={signUp_styles.mobile_description_point}>
                    개인정보 처리방침
                  </strong>
                  에 동의합니다.
                </label>
              </div>

              <div className={signUp_styles.mobile_checkbox_wrapper}>
                <input
                  type="checkbox"
                  id="option"
                  name="option"
                  className={signUp_styles.mobile_checkbox}
                />
                <label htmlFor="option" className={signUp_styles.mobile_checkbox_label}>
                  마케팅 정보 수신에 동의합니다. (선택)
                </label>
              </div>
            </div>
            {/* 모바일 회원가입 페이지 버튼 */}
            <div className={signUp_styles.mobile_buttons_wrapper}>
              <button className={`${styles.signup_button} `}>회원가입</button>
            </div>
          </form>
        </div>
        {/* 모바일 회원가입 페이지 조항 */}
        <span className={signUp_styles.mobile_terms}>
          회원가입 시 Sophies warehouse의 <br />
          <strong className={signUp_styles.mobile_description_point}>이용약관</strong> 및
          <strong className={signUp_styles.mobile_description_point}>개인정보처리방침</strong>에
          동의하는 것으로 간주됩니다.
        </span>
      </div>

      {/* 웹 회원가입 페이지 */}
      <div className={signUp_styles.web_signUp_page}>
        {/* 웹 회원가입 페이지 헤더 */}
        <Image
          className={signUp_styles.web_logo}
          src="/logo/소피의창고 로고.svg"
          alt="소피의 창고 로고"
          width={287}
          height={136}
        />
        <span className={signUp_styles.web_description}>
          &quot;아직 문은 잠겨 있어요. 열쇠는 &quot;
          <strong className={signUp_styles.web_description_point}>&quot;회원가입&quot;</strong>에
          있답니다.
        </span>

        {/* 웹 회원가입 페이지 박스 */}
        <div className={signUp_styles.web_form_wrapper}>
          {/* 웹 회원가입 페이지 뒤로가기와 회원가입 제목 */}
          <div className={signUp_styles.web_signUp_title_container}>
            <svg
              className={signUp_styles.web_back_button}
              width="28"
              height="28"
              viewBox="0 0 15 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              xlinkHref="http://www.w3.org/1999/xlink"
            >
              <rect
                x="15"
                y="15.6523"
                width="15"
                height="15"
                transform="rotate(180 15 15.6523)"
                fill="url(#pattern0_315_1191)"
              />
              <defs>
                <pattern
                  id="pattern0_315_1191"
                  patternContentUnits="objectBoundingBox"
                  width="1"
                  height="1"
                >
                  <use xlinkHref="#image0_315_1191" transform="scale(0.00195312)" />
                </pattern>
                <image
                  id="image0_315_1191"
                  width="512"
                  height="512"
                  preserveAspectRatio="none"
                  xlinkHref="data:image/png;base64,"
                />
              </defs>
            </svg>

            <span className={signUp_styles.web_signUp_title}>회원가입</span>
          </div>
          {/* 웹 회원가입 페이지 인풋 */}

          <form className={signUp_styles.web_form}>
            <div className={signUp_styles.web_label_top}>
              <div className={signUp_styles.web_input_name_wrapper}>
                <label className={signUp_styles.web_label} htmlFor="email">
                  이름 *
                </label>
                <input className={signUp_styles.web_input_name} type="text" id="name" />
              </div>
              <div className={signUp_styles.web_input_pn_wrapper}>
                <label className={signUp_styles.web_label} htmlFor="email">
                  휴대폰 번호 *
                </label>
                <input className={signUp_styles.web_input_pn} type="text" id="phonenumber" />
              </div>
            </div>

            <label className={signUp_styles.web_label} htmlFor="email">
              이메일 주소 *
            </label>
            <input
              className={signUp_styles.web_input}
              type="email"
              id="email"
              placeholder="example@example.com"
            />

            <label className={signUp_styles.web_label} htmlFor="password">
              비밀번호 *
            </label>
            <input className={signUp_styles.web_input} type="password" id="password" />

            <label className={signUp_styles.web_label} htmlFor="password">
              비밀번호 확인 *
            </label>
            <input className={signUp_styles.web_input} type="password" id="password" />

            <div className={signUp_styles.web_checkbox_container}>
              <div className={signUp_styles.web_checkbox_wrapper}>
                <input
                  type="checkbox"
                  id="option"
                  name="option"
                  className={signUp_styles.web_checkbox}
                />
                <label htmlFor="option" className={signUp_styles.web_checkbox_label}>
                  <strong className={signUp_styles.web_description_point}>이용약관</strong>에
                  동의합니다.
                </label>
              </div>

              <div className={signUp_styles.web_checkbox_wrapper}>
                <input
                  type="checkbox"
                  id="option"
                  name="option"
                  className={signUp_styles.web_checkbox}
                />
                <label htmlFor="option" className={signUp_styles.web_checkbox_label}>
                  <strong className={signUp_styles.web_description_point}>개인정보 처리방침</strong>
                  에 동의합니다.
                </label>
              </div>

              <div className={signUp_styles.web_checkbox_wrapper}>
                <input
                  type="checkbox"
                  id="option"
                  name="option"
                  className={signUp_styles.web_checkbox}
                />
                <label htmlFor="option" className={signUp_styles.web_checkbox_label}>
                  마케팅 정보 수신에 동의합니다. (선택)
                </label>
              </div>
            </div>
            {/* 웹 회원가입 페이지 버튼 */}
            <div className={signUp_styles.web_buttons_wrapper}>
              <button className={`${styles.signup_button} `}>회원가입</button>
            </div>
          </form>
        </div>
        {/* 웹 회원가입 페이지 조항 */}
        <span className={signUp_styles.web_terms}>
          회원가입 시 Sophies warehouse의
          <strong className={signUp_styles.web_description_point}>이용약관</strong> 및
          <strong className={signUp_styles.web_description_point}>개인정보처리방침</strong>에
          동의하는 것으로 간주됩니다.
        </span>
      </div>
    </>
  );
}
