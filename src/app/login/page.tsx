import '@/styles/globals.css';
import login_styles from '@/styles/login/login.module.css';
import styles from '@/styles/components/button.module.css';
import Link from 'next/link';
import Image from 'next/image';

export default function LoginPage() {
  return (
    <>
      {/* 모바일 로그인 페이지 */}
      <div className={login_styles.mobile_login_page}>
        {/* 모바일 로그인 페이지 헤더 */}
        <Image
          className={login_styles.mobile_logo}
          src="/logo/소피의창고 로고.svg"
          alt="소피의 창고 로고"
          width={173}
          height={82}
        />
        <span className={login_styles.mobile_description}>
          오래된 저택의 문이 열립니다...
          <br></br>
          <strong className={login_styles.mobile_description_point}>&quot;로그인&quot;</strong> 하면
          소피의 물건을 구매 할 수 있어요.
        </span>

        {/* 모바일 로그인 페이지 박스 */}
        <div className={login_styles.mobile_form_wrapper}>
          {/* 모바일 로그인 페이지 뒤로가기와 로그인 제목 */}
          <div className={login_styles.mobile_login_title_container}>
            <svg
              className={login_styles.mobile_back_button}
              width="15"
              height="15"
              viewBox="0 0 15 15"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              xlinkHref="http://www.w3.org/1999/xlink"
            >
              <rect
                x="15"
                y="15"
                width="15"
                height="15"
                transform="rotate(180 15 15)"
                fill="url(#pattern0_311_1505)"
              />
              <defs>
                <pattern
                  id="pattern0_311_1505"
                  patternContentUnits="objectBoundingBox"
                  width="1"
                  height="1"
                >
                  <use xlinkHref="#image0_311_1505" transform="scale(0.00195312)" />
                </pattern>
                <image
                  id="image0_311_1505"
                  width="512"
                  height="512"
                  preserveAspectRatio="none"
                  xlinkHref="data:image/png;base64,"
                />
              </defs>
            </svg>
            <span className={login_styles.mobile_login_title}>로그인</span>
          </div>
          {/* 모바일 로그인 페이지 인풋 */}
          <form className={login_styles.mobile_form}>
            <label className={login_styles.mobile_label} htmlFor="email">
              이메일 주소
            </label>
            <input
              className={login_styles.mobile_input}
              type="email"
              id="email"
              placeholder="example@example.com"
            />

            <label className={login_styles.mobile_label} htmlFor="password">
              비밀번호
            </label>
            <input className={login_styles.mobile_input} type="password" id="password" />

            <div className={login_styles.mobile_option}>
              <div className={login_styles.mobile_checkbox_container}>
                <input
                  type="checkbox"
                  id="option"
                  name="option"
                  className={login_styles.mobile_checkbox}
                />
                <label htmlFor="option" className={login_styles.mobile_checkbox_label}>
                  로그인 상태 유지
                </label>
              </div>
              <Link href="#" className={login_styles.mobile_forgot_password_link}>
                <span>비밀번호를 잊으셨나요?</span>
              </Link>
            </div>
            {/* 모바일 로그인 페이지 버튼 */}
            <div className={login_styles.mobile_buttons_wrapper}>
              <button className={`${styles.login_button} `}>로그인</button>
              <button className={`${styles.signup_button} `}>회원가입</button>
            </div>
          </form>
        </div>
        {/* 모바일 로그인 페이지 조항 */}
        <span className={login_styles.mobile_terms}>
          로그인하면 <strong className={login_styles.mobile_description_point}>이용약관</strong> 및
          <strong className={login_styles.mobile_description_point}>개인정보처리방침</strong>에
          동의하는 것으로 간주됩니다.
        </span>
      </div>

      {/* 웹 로그인 페이지 */}
      <div className={login_styles.web_login_page}>
        {/* 웹 로그인 페이지 헤더 */}
        <Image
          className={login_styles.web_logo}
          src="/logo/소피의창고 로고.svg"
          alt="소피의 창고 로고"
          width={287}
          height={136}
        />
        <span className={login_styles.web_description}>
          오래된 저택의 문이 열립니다...
          <br></br>
          <strong className={login_styles.web_description_point}>&quot;로그인&quot;</strong> 하면
          소피의 물건을 구매 할 수 있어요.
        </span>

        {/* 웹 로그인 페이지 박스 */}
        <div className={login_styles.web_form_wrapper}>
          {/* 웹 로그인 페이지 뒤로가기와 로그인 제목 */}
          <div className={login_styles.web_login_title_container}>
            <svg
              className={login_styles.web_back_button}
              width="30"
              height="30"
              viewBox="0 0 15 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              xlinkHref="http://www.w3.org/1999/xlink"
            >
              <rect
                x="15"
                y="15.6523"
                width="15"
                height="15"
                transform="rotate(180 15 15.6523)"
                fill="url(#pattern0_315_1191)"
              />
              <defs>
                <pattern
                  id="pattern0_315_1191"
                  patternContentUnits="objectBoundingBox"
                  width="1"
                  height="1"
                >
                  <use xlinkHref="#image0_315_1191" transform="scale(0.00195312)" />
                </pattern>
                <image
                  id="image0_315_1191"
                  width="512"
                  height="512"
                  preserveAspectRatio="none"
                  xlinkHref="data:image/png;base64,"
                />
              </defs>
            </svg>
            <span className={login_styles.web_login_title}>로그인</span>
          </div>
          {/* 웹 로그인 페이지 인풋 */}
          <form className={login_styles.web_form}>
            <label className={login_styles.web_label} htmlFor="email">
              이메일 주소
            </label>
            <input
              className={login_styles.web_input}
              type="email"
              id="email"
              placeholder="example@example.com"
            />

            <label className={login_styles.web_label} htmlFor="password">
              비밀번호
            </label>
            <input className={login_styles.web_input} type="password" id="password" />

            <div className={login_styles.web_option}>
              <div className={login_styles.web_checkbox_container}>
                <input
                  type="checkbox"
                  id="option"
                  name="option"
                  className={login_styles.web_checkbox}
                />
                <label htmlFor="option" className={login_styles.web_checkbox_label}>
                  로그인 상태 유지
                </label>
              </div>
              <Link href="#" className={login_styles.web_forgot_password_link}>
                <span>비밀번호를 잊으셨나요?</span>
              </Link>
            </div>
            {/* 웹 로그인 페이지 버튼 */}
            <div className={login_styles.web_buttons_wrapper}>
              <button className={`${styles.login_button} `}>로그인</button>
              <button className={`${styles.signup_button} `}>회원가입</button>
            </div>
          </form>
        </div>
        {/* 웹 로그인 페이지 조항 */}
        <span className={login_styles.web_terms}>
          로그인하면 <strong className={login_styles.web_description_point}>이용약관</strong> 및
          <strong className={login_styles.web_description_point}>개인정보처리방침</strong>에
          동의하는 것으로 간주됩니다.
        </span>
      </div>
    </>
  );
}
